...
export const getShopProducts = async (req, res, next) => {
  try {
    ...
    console.log(req.params.id);
    // const { data } = await Product.find({ shopLiscNo });
    const data  = await Product.find({ shopLiscNo:req.params.id });//{data} not write as in js we write like this when we have to extract/diffract data field from result since it is array there are multiple obj so it wont work

    // const data=await Product.find({});
    //  const data = await Shop.findOne({ shopLiscNo:req.params.id });
    // const { data } = await Shop.find();

    console.log(data);
    if (!data) {
      return next(new ErrorHandler("Currently No product available!", 404));
    }
    res.status(201).json({
      sucess: true,
      data,
